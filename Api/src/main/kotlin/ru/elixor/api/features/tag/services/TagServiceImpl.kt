...
import ru.elixor.api.features.tag.dto.toDto
import java.util.*


@Service
class TagServiceImpl(private val folderRepository: TagRepository) : TagService {
    override fun getAll(jwt: Jwt): List<TagOutputDto> {
        return folderRepository.findAllByUserUid(UUID.fromString(jwt.subject)).map { it.toDto() }
    }
